...
<div class="relative">
    ...
    <Dropdown
        ...
        class="absolute left-0 mt-2 w-48 bg-white rounded-md shadow-lg dark:bg-gray-700"
    >
        <div class="px-4 py-2">
            <input
                type="text"
                ...
                class="w-full px-4 py-2 mb-4 border rounded-md focus:outline-none focus:ring focus:border-blue-300 dark:bg-gray-800 dark:border-gray-600 dark:text-white"
            />
        </div>
        {#if currentUsersFriends.length > 0}
            {#each currentUsersFriends as friend}
                <DropdownItem
                    ...
                    class="{selectedFriends.includes(friend)
                        ? 'bg-gray-200'
                        : ''} px-4 py-2 text-sm text-gray-600 hover:bg-gray-100 hover:text-gray-900 dark:text-white cursor-pointer"
                >
                    {friend.username}
                </DropdownItem>
            {/each}
        ...
        <DropdownItem
            ...
            class="px-4 py-2 text-sm text-gray-600 hover:bg-gray-100 hover:text-gray-900 dark:text-white cursor-pointer"
        >
            Create Group
        </DropdownItem>
    </Dropdown>
