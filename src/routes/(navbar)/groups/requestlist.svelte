<script lang="ts">
    ...
    const acceptRequest = async (group: {
        id: number;
        user_id: number;
        group_id: number;
        groups: { name: string };
    ...
    const declineRequest = async (group: {
        id: number;
        user_id: number;
        group_id: number;
        groups: { name: string };
...
{#if groupRequests.length > 0 || groupInvites.length > 0}
    <Listgroup active class="w-full md:w-80">
        ...
        {#each filteredGroups as group}
            <ListgroupItem
                class="flex flex-col md:flex-row items-start md:items-center justify-between text-base font-semibold gap-2"
            >
                <div>
                    <a
                        rel="external"
                        href="../group/{group.users.username}"
                        class="flex items-center font-semibold text-gray-900 dark:text-white"
                    >
                        <span
                            >{group.users.username.length > 15
                                ? `${group.users.username.slice(0, 15)}...`
                                : group.users.username}</span
                        >
                    </a>
                    <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">
                        Group: {group.groups.name.length > 15
                            ? `${group.groups.name.slice(0, 15)}...`
                            : group.groups.name}
                    </p>
                </div>
                <div class="flex gap-2">
                    <button
                        on:click={() => acceptRequest(group)}
                        class="flex items-center p-1 text-sm font-medium text-green-600 hover:bg-gray-100 dark:hover:bg-gray-600 dark:text-green-500 hover:underline rounded-b-lg"
                    >
                        Accept
                    </button>
                    <button
                        on:click={() => declineRequest(group)}
        ...
        {#each filteredInvites as invite}
            ...
            >
                <div>
                    <a
                        rel="external"
                        href="../group/{invite.groups.name}"
                        class="flex items-center font-semibold text-gray-900 dark:text-white"
                    >
                        <span
                            >{invite.groups.name.length > 15
                                ? `${invite.groups.name.slice(0, 15)}...`
                                : invite.groups.name}</span
                        >
                    </a>
                    <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">
                        From: {invite.groups.creator.length > 15
                            ? `${invite.groups.creator.slice(0, 15)}...`
                            : invite.groups.creator}
                    </p>
                </div>
                <div class="flex gap-2">
                    <button
                        on:click={() => acceptRequest(invite)}
                        class="flex items-center p-1 text-sm font-medium text-green-600 hover:bg-gray-100 dark:hover:bg-gray-600 dark:text-green-500 hover:underline rounded-b-lg"
                    >
                        Join
                    </button>
                    <button
                        on:click={() => declineRequest(invite)}
                        class="flex items-center p-1 text-sm font-medium text-red-600 hover:bg-gray-100 dark:hover:bg-gray-600 dark:text-red-500 hover:underline rounded-b-lg"
                    >
                        Decline
                    </button>
