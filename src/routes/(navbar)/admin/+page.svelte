<script lang="ts">
    ...
    if (users) valueCount++;
    if (relationships)
        itemTuples.push({ label: 'Relationships', value: valueCount, component: Relationship, prop: relationships });
    ...
    if (group_routes)
        itemTuples.push({ label: 'Group Routes', value: valueCount, component: GroupRoutes, prop: group_routes });
    if (group_routes) valueCount++;
    if (route_coordinates)
        itemTuples.push({
            label: 'Route Coordinates',
            value: valueCount,
            component: RouteCoordinates,
            prop: route_coordinates,
        });
