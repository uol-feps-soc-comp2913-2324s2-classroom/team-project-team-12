        ...
        {#if currentPage == 0}
            ...
            <div class="topButtons">
                <Button
                    ...
                    on:click={() => updateCurrentPage('Your Routes')}>Your Routes</Button
                >
                <Button
                    color="alternative"
                    pill
                    style="margin-right: 10px"
                    on:click={() => updateCurrentPage('Friends Routes')}>Friends Routes</Button
                >
                <Button
                    color="alternative"
                    pill
                    style="margin-right: 10px"
                    on:click={() => updateCurrentPage('Public Routes')}>Public Routes</Button
                >
                {#each groupRouteEntries as groupRouteEntry (groupRouteEntry)}
                    <Button
                        ...
                        pill
                        style="margin-right: 10px"
                        on:click={() => updateCurrentPage(groupRouteEntry.group_name)}
                        >{groupRouteEntry.group_name}</Button
                    >
                {/each}
            </div>
            <RouteHorizontalScroll
                user_id={userData.id}
                nameOfList="Your Routes"
                routeEntries={userRouteEntries}
                {adminGroupNames}
                {addRouteToGroup}
                {searchTerm}
                showGroupOnMap={false}
                {updateCurrentPage}
            />
            {#if friendsRouteEntries.length > 0}
                <RouteHorizontalScroll
                    user_id={userData.id}
                    nameOfList="Friends Routes"
                    routeEntries={friendsRouteEntries}
                    {adminGroupNames}
                    {addRouteToGroup}
                    {searchTerm}
                    showGroupOnMap={false}
