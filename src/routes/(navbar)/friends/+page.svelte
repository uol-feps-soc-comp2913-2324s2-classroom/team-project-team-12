...
<Input bind:value={searchTerm} placeholder="Search..." on:input={() => {}} />

<div class="page-container">
    {#if currentUserFriends.length > 0}
        <div class="friends-container">
            <FriendList {currentUserFriends} {searchTerm} />
        </div>
    {/if}
    {#if friendRequests.length > 0}
        <div class="requests-container">
            <RequestsList {friendRequests} {searchTerm} />
        </div>
    {/if}
    {#if unaddedPeople.length > 0 || requested.length > 0}
        <div class="unadded-people-container">
            <UnaddedList people={unaddedPeople} {requested} {searchTerm} />
        </div>
    {/if}
</div>
